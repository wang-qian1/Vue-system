<template>
  <div style="height: 100%">
    <!--logo栏位-->
    <div v-if="nav.LOGO && this.nav.LOGO.id === '1'" class="avatar avatar-flex"
    ...
    <el-menu :default-active="String(activeNav)" class="el-menu-vertical" :collapse="getCollapseState"
      :background-color="getAsideColor" :text-color="getATextColor" :active-text-color="getActiveColor"
      :unique-opened=getDoubleOpen :default-openeds="defaultOpen" ref="menu" router>
      <!--动态生成sideItem-->
      <template v-for="(item, parentIndex) in nav.nav">
        <SideNav :item="item" :index="parentIndex" :state="getCollapseState" :key="parentIndex"></SideNav>
      </template>
    </el-menu>
  </div>
</template>
